...
class GroudTruthGenerator:
    ...
    def __init__(self, labeler='', labeling_model='', experiment_start='', experiment_end ='', event_period='', \
        ...
        #if self.class_dict['invalid_class_value']:
            #del self.class_dict['invalid_class_value']
        self.default_value = self.class_dict['default_class_value']
        #if self.class_dict['default_class_value']:
            #del self.class_dict['default_class_value']
            ...
            for serie in self.class_dict.keys():
                ...
                for a in self.class_dict[serie]['periods']:
                    if event_start < a[0] and event_end > a[1]:
                        ...
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" is fully included in the event: "+str(event_start)+" - "+str(event_end)
                        ...
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" is partially included (using a safety band on the right) in "+str(event_start)+" - "+str(event_end)
                        event_msg.append(msg)
                        #break
                    elif event_start-self.guardperiod < a[0] and event_end > a[1]:
                        event_labels.append(self.class_dict[serie]['value'])
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" is partially included (using a safety band on the left) in "+str(event_start)+" - "+str(event_end)
                        event_msg.append(msg)
                        #break
                    elif event_start > a[0] and event_end < a[1]:
                        event_labels.append(self.class_dict[serie]['value'])
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" includes the event "+str(event_start)+" - "+str(event_end)
                        event_msg.append(msg)
                        #break
                    elif event_start > a[0]-self.guardperiod and event_end < a[1]:
                        event_labels.append(self.class_dict[serie]['value'])
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" partially includes (using a safety band on the left) the event "+str(event_start)+" - "+str(event_end)
                        event_msg.append(msg)
                        #break
                    elif event_start > a[0] and event_end < a[1]+self.guardperiod:
                        event_labels.append(self.class_dict[serie]['value'])
                        msg = "The "+serie+" period "+str(a[0])+" - "+str(a[1])+" partially includes (using a safety band on the right) the event "+str(event_start)+" - "+str(event_end)
                        event_msg.append(msg)
                        #break
