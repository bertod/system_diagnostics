from AnomalyDetector import GroundTruthGenerator
...
class Test:
    ...
    def get_df_events_index(self, df_events):
        print('\n---Building the Event DateTimeIndex: STARTED')
        events_start = []
        ...
        print('\n---Building the Event DateTimeIndex: DONE')
            ...
            if truth['labeler_alias'].lower().strip() == labeler.lower().strip():
                ...
                for model in truth['labeling_models']:  # filter the session
                    if model['model_name'].lower().strip() == labeling_model.lower().strip():
                        ...
                        groundtruths_dict[truth['labeler_alias']][model['model_name']]['invalid_class_value'] = \
                            model['invalid_class_value']
                        periods_list = []
                        ...
                        groundtruths_dict[truth['labeler_alias']][model['model_name']]['period']['periods'] = \
                            periods_list
    ...
    def test_anomaly_detector(self):
        ...
        # self.gtg_instance = GroundTruthGenerator('Peter', labeling_model, "2019-02-04 00:00:01", "2019-02-10 00:59:59",
        # "15m","60", "Europe/Rome", df_events_index, groundtruths_dict)
        
        self.gtg_instance = GroundTruthGenerator(labeler, labeling_model, "2019/11/01 00:00:01", "2019/11/12 23:59:59",
                                                "15m", "60", "Europe/Rome", df_events_index, groundtruths_dict)
        ...
        plt.yticks(yticks)
        plt.ylim(min_y, max_y)
        self.gtg_instance.gt_series_events_label.plot(label='Event (GT signal method) signal label',
                                                      c='red', markersize=20)
        ...
        plt.yticks(yticks)
        plt.ylim(min_y, max_y)
        self.gtg_instance.series_events_label.plot(label='Event (inclusion method) signal label',
                                                   c='red', markersize=20)
        ...
        plt.bar(x_even, [1]*len(x_even), width=2, alpha=0.8, color=color_list_even,
                align='edge', edgecolor='#66594C', zorder=10)
        plt.bar(x_odd, [2]*len(x_odd), width=2, alpha=0.5, color=color_list_odd,
                align='edge', edgecolor='#66594C', zorder=1)
